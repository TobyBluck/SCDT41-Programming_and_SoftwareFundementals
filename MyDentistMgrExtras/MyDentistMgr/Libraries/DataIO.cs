...
namespace MyDentistMgr.Libraries
{
    static class DataIO //static classes cannot be instantiated or extended
    {
        ...
        public static string practicePath = rootPath + @"\" + MyDentistMgr.Default.DentalPracticePath; //creates a path to the dental practice directory.
        ...
        */
        public static void loadPractices()
        {
            ...
            StreamReader practiceInfoFile;
            string[] practiceDirectories;
            ...
            try
            {
                ...
                for (int i = 0; i < practiceDirectories.Length; i ++)
                {
                    practiceInfoFile = new StreamReader(practiceDirectories[i] + "\\info.txt");//iterating over every folder/ over datafolder and look through the info.txt and create the rooms folder if non exsitent 
        ...
        public static void loadUsers()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < userFiles.Length; i ++) //iterates over every file in the user directory.
                {
                    ...
                    {
                        case "Administrator":
                            newUser = new Administrator(username, userPassword, name); //As administrator extends User, it can be held as a User but must be casted. When storing this as a user, only user members will be available. This can be casted back at any point to access Admin features. While stored as a user, the overridden functions will remain overridden. This is called polymorphism. https://docs.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/polymorphism
                            break;
                        case "Receptionist":
                        ...
                        case "DentistNurse":
                            ...
                            break;
                        default:
                            newUser = new DentistNurse("", "", "", "", ""); //This is to prevent compiler error when attempting to add a potentially ininitialized User variable. Due to the success variable. 
