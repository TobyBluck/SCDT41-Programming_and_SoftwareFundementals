       ...
       */
        ...
        public static List<Staff> getStaff()
        {
            ...
            try
            {
                for(int i = 0; i < users.Count; i ++)
                {
                    ...
                    {
                        staff.Add((Staff) users[i]); //Cast the user into a staff object and adds it t the staff list.
                    }
                }
            }
            catch (Exception e)
        ...
        */
        public static List<Receptionist> getReceptionists()
        {
            ...
            try
            {
                for (int i = 0; i < users.Count; i++)
                {
                    if (users[i] is Receptionist)//runs through users ans find anyone that is a receptionist 
                    {
                        receptionists.Add((Receptionist)users[i]); //Cast the user into a staff object and adds it to the staff list.
                    }
                }
            }
            catch (Exception e)
        ...
        */
        public static List<Appointment> getPractitionerAppointments(DentistNurse practitioner)
        {
            ...
            try
            {
                practitionerAppointments = allAppointments.FindAll(appointment => appointment.getRoom().getDentist().Equals(practitioner) || appointment.getRoom().getNurse().Equals(practitioner)); //Gets all appointments for the praticoner or nurse. || is an OR operator which runs for the nurse. .equals is used for objects but is the same as == operator 
            }
            catch (Exception e)
            {
